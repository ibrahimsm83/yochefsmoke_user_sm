...
class PaymentContainer extends StatelessWidget {
  ...
  final String? text2;
  final String icon;
  const PaymentContainer({Key? key,this.selected=false,this.onTap,this.text1="",
    required this.icon,
  ...
  Widget build(BuildContext context) {
    ...
    return GestureDetector(
      ...
      child: ShadowContainer(
        radius: radius,
        child: Container(padding: EdgeInsets.symmetric(vertical: AppSizer.getHeight(10),
            horizontal: AppSizer.getWidth(10)),
          decoration: const BoxDecoration(color: AppColor.COLOR_WHITE),
          child: Row(children: [
            Container(
              padding: EdgeInsets.all(padd),
              width: imgHeight,height: imgHeight,decoration: BoxDecoration(
                borderRadius: BorderRadius.circular(radius),
                color: AppColor.COLOR_GREY9
            ),child: CustomImage(image:icon, imageType: ImageType.TYPE_ASSET,),),
            ...
            CircularButton(diameter: diameter, icon: selected?AssetPath.ICON_TICK:null,
...
class AddVoucherButton extends CustomButton{
  ...
  AddVoucherButton({Function()? onTap,}):super(fontsize: 15,bgColor: AppColor.COLOR_GREY1,
      radius: AppSizer.getRadius(10),text: AppString.TEXT_ADD_VOUCHER,onTap: onTap,
      fontWeight: FontWeight.w600,textColor: AppColor.COLOR_RED1);

  @override
  Widget build(BuildContext context) {
    return DottedContainer(radius: radius!,color: AppColor.COLOR_GREY8,
        dashlength: 10,borderWidth: 2,
  ...
  Widget get child {
    return Row(
      ...
      children: [
      IconVoucher(size: AppSizer.getHeight(25),),
