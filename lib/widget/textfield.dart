...
mixin LoginFieldProps {
  ...
  late Color bgColor,hintColor,textColor,focusedBorderColor;
  int? maxLength;
  late bool expands;
...
class CustomField extends StatelessWidget with LoginFieldProps{
  ...
  //  final double font=AppSizer.getFontSize(fontsize);
    ...
    return InputDecoration(
      ...
      //  prefixIcon: Container(width: 1000,height: 120,color: Colors.red,),
...
class _CustomPasswordFieldState extends State<CustomPasswordField>{
  ...
  Widget build(BuildContext context) {
    return CustomField(controller: widget.controller,prefixIcon: widget.prefixIcon,
      hidden: widget.hidden,border: widget.border,bgColor: widget.bgColor,
  ...
  Widget buildSuffixIcon(){
   // final double iconsize=AppSizer.getFontSize((widget.fontsize+8));
    ...
    return Padding(
      ...
      child: GestureDetector(onTap: toggleHidden,
          child:Container(color: AppColor.COLOR_TRANSPARENT,
              width: iconsize,height: iconsize,
              child: Center(
                child: CustomMonoIcon(
            icon: widget.hidden?AssetPath.ICON_EYE:AssetPath.ICON_EYE_CROSS,
